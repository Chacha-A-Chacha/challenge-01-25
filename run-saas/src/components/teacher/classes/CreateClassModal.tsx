"use client";

import { useState } from "react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
  DialogFooter,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { AlertCircle, Loader2 } from "lucide-react";
import { useClassActions } from "@/store/teacher/class-store";
import { CLASS_RULES } from "@/lib/constants";

interface CreateClassModalProps {
  open: boolean;
  onClose: () => void;
}

export function CreateClassModal({ open, onClose }: CreateClassModalProps) {
  const { createClass, isCreating, error, clearErrors } = useClassActions();
  const [name, setName] = useState("");
  const [capacity, setCapacity] = useState(CLASS_RULES.DEFAULT_CAPACITY.toString());
  const [validationError, setValidationError] = useState<string | null>(null);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setValidationError(null);
    clearErrors();

    // Validate name
    if (!name.trim()) {
      setValidationError("Class name is required");
      return;
    }
    if (name.trim().length < 2) {
      setValidationError("Class name must be at least 2 characters");
      return;
    }
    if (name.trim().length > 50) {
      setValidationError("Class name must be at most 50 characters");
      return;
    }

    // Validate capacity
    const capacityNum = parseInt(capacity, 10);
    if (isNaN(capacityNum) || capacityNum < 1) {
      setValidationError("Capacity must be at least 1");
      return;
    }
    if (capacityNum > CLASS_RULES.MAX_CAPACITY) {
      setValidationError(`Capacity cannot exceed ${CLASS_RULES.MAX_CAPACITY}`);
      return;
    }

    const result = await createClass(name.trim(), capacityNum);
    if (result) {
      handleClose();
    }
  };

  const handleClose = () => {
    setName("");
    setCapacity(CLASS_RULES.DEFAULT_CAPACITY.toString());
    setValidationError(null);
    clearErrors();
    onClose();
  };

  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Create New Class</DialogTitle>
          <DialogDescription>
            Add a new class to your course. You can add sessions after creating
            the class.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4">
          {(validationError || error) && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>{validationError || error}</AlertDescription>
            </Alert>
          )}

          <div className="space-y-2">
            <Label htmlFor="name">Class Name</Label>
            <Input
              id="name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g., Class A, Morning Group"
              disabled={isCreating}
              autoFocus
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor="capacity">Capacity</Label>
            <Input
              id="capacity"
              type="number"
              value={capacity}
              onChange={(e) => setCapacity(e.target.value)}
              min={1}
              max={CLASS_RULES.MAX_CAPACITY}
              disabled={isCreating}
            />
            <p className="text-xs text-muted-foreground">
              Maximum number of students (up to {CLASS_RULES.MAX_CAPACITY})
            </p>
          </div>

          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={handleClose}
              disabled={isCreating}
            >
              Cancel
            </Button>
            <Button type="submit" disabled={isCreating}>
              {isCreating && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
              Create Class
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
}
