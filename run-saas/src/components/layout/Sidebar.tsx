...
export function Sidebar() {
    ...
    const isHeadTeacher = user.role === USER_ROLES.TEACHER && user.teacherRole === TEACHER_ROLES.HEAD
    const isAdditionalTeacher = user.role === USER_ROLES.TEACHER && user.teacherRole === TEACHER_ROLES.ADDITIONAL
    ...
    const headTeacherNav: NavItem[] = [
        ...
        { label: 'My Course', href: '/teacher/course', icon: GraduationCap },
        { label: 'Teachers', href: '/teacher/teachers', icon: Users },
        { label: 'Classes', href: '/teacher/classes', icon: School },
        { label: 'Sessions', href: '/teacher/sessions', icon: Calendar },
            ...
            <div className="p-6 border-b">
                <div className="flex items-center gap-3">
                    <div className={cn(
                        ...
                        portalConfig.color === 'emerald' ? 'bg-emerald-600' : 'bg-blue-600'
                    )}>
                    ...
                    <div>
                        ...
                        <p className={cn(
                            "text-xs font-medium",
                            portalConfig.color === 'emerald' ? 'text-emerald-700' : 'text-blue-700'
                        )}>
                            {portalConfig.label}
                        </p>
                    </div>
                            ...
                            >
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        active && portalConfig.color === 'emerald' && "bg-emerald-100 text-emerald-900 hover:bg-emerald-100",
                                        active && portalConfig.color === 'blue' && "bg-blue-100 text-blue-900 hover:bg-blue-100"
                                    )}
                                >
                                    <Icon className={cn(
                                        "w-5 h-5",
                                        active && portalConfig.color === 'emerald' ? "text-emerald-700" :
                                        active && portalConfig.color === 'blue' ? "text-blue-700" :
                                        "text-gray-600"
                                    )} />
            ...
            <div className="p-4 border-t">
                <div className="flex items-center gap-3">
                    <div className={cn(
                        ...
                        portalConfig.color === 'emerald' ? 'bg-emerald-100' : 'bg-blue-100'
                    )}>
                        <span className={cn(
                            "font-semibold text-sm",
                            portalConfig.color === 'emerald' ? 'text-emerald-700' : 'text-blue-700'
                        )}>
                            {user.email?.charAt(0).toUpperCase() || user.firstName?.charAt(0).toUpperCase() || 'U'}
                        </span>
                    </div>
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-600">
                            {isAdmin ? 'Administrator' :
                             isHeadTeacher ? 'Head Teacher' :
                             isAdditionalTeacher ? 'Teacher' :
                             'Student'}
                        </p>
                    </div>
                </div>
