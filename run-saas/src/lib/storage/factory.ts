  ...
  switch (provider) {
    ...
    case 's3':
      if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY || !process.env.AWS_S3_BUCKET) {
        throw new Error('Missing required S3 environment variables: AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_S3_BUCKET');
      }
      config.s3 = {
    ...
    case 'gcs':
      if (!process.env.GCS_PROJECT_ID || !process.env.GCS_BUCKET) {
        throw new Error('Missing required GCS environment variables: GCS_PROJECT_ID, GCS_BUCKET');
      }
      config.gcs = {
    ...
    case 'azure':
      if (!process.env.AZURE_STORAGE_CONNECTION_STRING || !process.env.AZURE_CONTAINER_NAME) {
        throw new Error('Missing required Azure environment variables: AZURE_STORAGE_CONNECTION_STRING, AZURE_CONTAINER_NAME');
      }
      config.azure = {
...
export function createStorageProvider(config: StorageConfig): IStorageProvider {
  ...
  switch (config.provider) {
    ...
    case 'azure':
      throw new Error('Azure provider not yet implemented. Run: npm install @azure/storage-blob');

    case 'cloudinary':
      throw new Error('Cloudinary provider not yet implemented. Run: npm install cloudinary');
