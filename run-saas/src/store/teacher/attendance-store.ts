...
function calculateAttendanceStats(records: AttendanceRecordWithDates[]) {
  ...
  const wrongSessionCount = records.filter(
  ...
  persist(
    (set, get) => ({
      ...
      loadSessionAttendance: async (sessionId) => {
        ...
        try {
          const response = await fetchWithTimeout(
            `${API_ROUTES.ATTENDANCE_SCAN}/session/${sessionId}`,
          );
          ...
          if (result.success && result.data) {
            ...
            const recordsWithDates =
              transformAttendanceRecords(attendanceRecords);
            ...
            throw new Error(
              result.error || "Failed to load session attendance",
            );
          }
        } catch (error) {
          const errorMessage =
      ...
      markManualAttendance: async (studentId, status) => {
        ...
        try {
          const response = await fetchWithTimeout(
            `${API_ROUTES.ATTENDANCE_SCAN}/manual`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
      ...
      bulkMarkAbsent: async (studentIds) => {
        ...
        try {
          const response = await fetchWithTimeout(
            `${API_ROUTES.ATTENDANCE_SCAN}/bulk`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
          ...
          if (result.success && result.data) {
            ...
            const attendanceRecordsWithDates = transformAttendanceRecords(
              result.data,
            );
            ...
            studentIds.forEach((studentId) => {
              updatedRecords = updateAttendanceRecord(
                updatedRecords,
                studentId,
                "ABSENT",
              );
          ...
          const errorMessage =
            error instanceof Error ? error.message : "Bulk attendance failed";
          set({
            error: errorMessage,
            isMarkingAttendance: false,
          });
      ...
      getAttendanceRate: () => {
        const { currentSessionData } = get();
        if (!currentSessionData || currentSessionData.totalStudents === 0)
          return 0;

        return Math.round(
          (currentSessionData.presentCount / currentSessionData.totalStudents) *
            100,
        );
      },
      ...
      getPresentStudents: () => {
        ...
        return currentSessionData.attendanceRecords.filter(
...
export function useSessionAttendance() {
  const currentSessionData = useAttendanceStore(
    (state) => state.currentSessionData,
  );
  const isLoadingSession = useAttendanceStore(
    (state) => state.isLoadingSession,
  );
  const getAttendanceRate = useAttendanceStore(
    (state) => state.getAttendanceRate,
  );
  const getPresentStudents = useAttendanceStore(
    (state) => state.getPresentStudents,
  );
  const getAbsentStudents = useAttendanceStore(
    (state) => state.getAbsentStudents,
  );
  const loadSessionAttendance = useAttendanceStore(
    (state) => state.loadSessionAttendance,
  );
  ...
  return {
    ...
    loadSession: loadSessionAttendance,
  };
}
...
export function useQRScanner() {
  const scanState = useAttendanceStore((state) => state.scanState);
  const isMarkingAttendance = useAttendanceStore(
    (state) => state.isMarkingAttendance,
  );
  const scanQRCode = useAttendanceStore((state) => state.scanQRCode);
  const startScanning = useAttendanceStore((state) => state.startScanning);
  const stopScanning = useAttendanceStore((state) => state.stopScanning);
  const clearScanError = useAttendanceStore((state) => state.clearScanError);
...
export function useAttendanceActions() {
  ...
  const markManualAttendance = useAttendanceStore(
    (state) => state.markManualAttendance,
  );
  const bulkMarkAbsent = useAttendanceStore((state) => state.bulkMarkAbsent);
  const error = useAttendanceStore((state) => state.error);
  const clearErrors = useAttendanceStore((state) => state.clearErrors);

  return {
    isMarkingAttendance,
    markManual: markManualAttendance,
    bulkMarkAbsent,
    error,
    clearErrors,
  };
