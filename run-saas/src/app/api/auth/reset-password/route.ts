...
export async function POST(request: NextRequest) {
    try {
        const { token, password, userType } = await request.json() as ResetPasswordRequest
        ...
        // Find user by token
        let user: { id: string; email: string; resetTokenExpiry: Date | null; firstName?: string } | null = null

        if (userType === 'student') {
            user = await prisma.student.findFirst({
                where: { resetToken: token },
                select: { id: true, email: true, resetTokenExpiry: true, firstName: true }
            })
        ...
        const html = passwordChangedTemplate({
            name: 'firstName' in user ? user.firstName : 'User',
