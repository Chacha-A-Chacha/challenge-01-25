...
import { approveReassignmentRequest } from "@/lib/db";
...
export async function POST(
  ...
  try {
    ...
    // Both head and additional teachers can approve reassignment requests
    ...
    return NextResponse.json<ApiResponse<{ message: string }>>({
      success: true,
      data: { message: "Reassignment request approved successfully" },
    });
  } catch (error) {
    ...
    return NextResponse.json<ApiResponse<null>>(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to approve reassignment request",
      },
      { status: 500 },
    );
  }
