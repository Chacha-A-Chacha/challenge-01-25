  ...
  try {
    ...
    if (!course || course.status !== "ACTIVE") {
      return NextResponse.json(
        {
          success: false,
          error: ERROR_MESSAGES.REGISTRATION.COURSE_NOT_ACTIVE,
        },
        { status: 400 },
      );
    }
    ...
    if (!classData) {
      return NextResponse.json(
        {
          success: false,
          error: "Class not found or does not belong to the selected course",
        },
        { status: 400 },
      );
    }
    ...
    if (
      saturdaySession.classId !== data.classId ||
      sundaySession.classId !== data.classId
    ) {
      return NextResponse.json(
    ...
    const response: ApiResponse<StudentRegistrationResponse> = {
      ...
      data: {
        ...
        saturdaySession: formatSessionTime(
          registration.saturdaySession.startTime,
          registration.saturdaySession.endTime,
        ),
        sundaySession: formatSessionTime(
          registration.sundaySession.startTime,
          registration.sundaySession.endTime,
        ),
 ...
 */
async function getSessionWithAvailability(
  sessionId: string,
  expectedDay: "SATURDAY" | "SUNDAY",
) {
